...
import { customAgentLoader, CustomAgentDefinition } from './custom-agent-loader.js';

/**
 * Built-in agent type definition
 */
export interface BuiltInAgentType {
  name: string;
  type: string;
  description: string;
  capabilities: string[];
...
/**
 * Unified agent entry (can be either built-in or custom)
 */
export type UnifiedAgentEntry = CustomAgentDefinition | BuiltInAgentType;

/**
 * Built-in agent types with default capabilities
 * These are the hardcoded types that exist without .md files
 */
const BUILT_IN_AGENTS: BuiltInAgentType[] = [
  {
    name: 'coordinator',
    type: 'coordinator',
...
class UnifiedAgentRegistry {
  private builtInAgents: Map<string, BuiltInAgentType>;
   ...
   */
  async getAgent(name: string): Promise<UnifiedAgentEntry | null> {
   ...
   * Custom agents override built-in agents with the same name
   */
  async getAllAgents(): Promise<UnifiedAgentEntry[]> {
    await this.ensureInitialized();

    const result = new Map<string, UnifiedAgentEntry>();
   ...
   */
  getBuiltInAgents(): BuiltInAgentType[] {
   ...
   */
  async searchAgents(query: string): Promise<UnifiedAgentEntry[]> {
   ...
   * Get agents by type
   */
  async getAgentsByType(type: string): Promise<UnifiedAgentEntry[]> {
    const allAgents = await this.getAllAgents();
   ...
   */
  async getAgentSystemPrompt(name: string): Promise<string | null> {
    const customAgent = await customAgentLoader.getAgent(name);
   ...
   */
  async getAgentHooks(name: string): Promise<{ pre?: string; post?: string } | null> {
    const customAgent = await customAgentLoader.getAgent(name);
   ...
   */
  async isCustomAgent(name: string): Promise<boolean> {
   ...
   */
  async getAgentSourcePath(name: string): Promise<string | null> {
    const customAgent = await customAgentLoader.getAgent(name);
...
// Convenience exports
export const getAgent = (name: string) => unifiedAgentRegistry.getAgent(name);
export const hasAgent = (name: string) => unifiedAgentRegistry.hasAgent(name);
export const getAllAgents = () => unifiedAgentRegistry.getAllAgents();
export const getAgentNames = () => unifiedAgentRegistry.getAgentNames();
export const searchAgents = (query: string) => unifiedAgentRegistry.searchAgents(query);
export const getAgentCapabilities = (name: string) => unifiedAgentRegistry.getAgentCapabilities(name);
export const getAgentSystemPrompt = (name: string) => unifiedAgentRegistry.getAgentSystemPrompt(name);
export const getAgentHooks = (name: string) => unifiedAgentRegistry.getAgentHooks(name);
export const refreshRegistry = () => unifiedAgentRegistry.refresh();
export const getRegistryStats = () => unifiedAgentRegistry.getStats();
export const isCustomAgent = (name: string) => unifiedAgentRegistry.isCustomAgent(name);
